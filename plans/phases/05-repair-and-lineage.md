...
- Implement the repair menu with these choices:
  ...
  - New tab forked from the previous Luna execution thread.
...
- Sol-fix repair:
  ...
  - Name the shared-worktree risk and require confirmation that no Luna tab is currently running an editing turn. Do not claim to detect
    idleness, add a writer lock, or treat an open TUI process as proof of activity.
- Re-run the Pareto model recommendation and user confirmation for every Luna repair launch.
...
- Forking Luna reuses context through a new thread; sending a prompt into the existing live Luna TUI is not supported.
...
- Luna-context fork with missing, closed, or ambiguous execution thread.
- Sol-fix requested from Plan mode and Default mode.
- Sol-fix accepted/rejected with another Luna tab still open, including the explicit idle-turn confirmation.
...
- The original Sol planning thread remains available throughout Luna repair paths.
